...
class GraphINN(InvertibleModule):
    ...
    def forward(self, x_or_z: Union[Tensor, Iterable[Tensor]],
        ...
        jacobian = torch.zeros(x_or_z[0].shape[0], dtype=x_or_z[0].dtype).to(x_or_z[0])
        outs = {}
    ...
    def _check_output(self, node, mod_out, jac, rev):
        ...
        if not torch.is_tensor(mod_jac):
            if isinstance(mod_jac, (float, int)):
                mod_jac = torch.zeros(out[0].shape[0],dtype=mod_out[0][0].dtype).to(out[0].device) \
                          + mod_jac
            elif jac:
                raise ValueError(
                ...
                raise ValueError(
                    ...
                    f"Jacobian: {mod_jac}")
        # if torch.any(torch.isnan(out[0])):
